...
local CharacterCard = require(Frames.CharacterCard)
...
function Characters(_, hooks)
	...
	local CharacterReducer = RoduxHooks.useSelector(hooks, function(state)
	...
	return Roact.createElement("Frame", {
		...
		Content = Blue_Background({
			title = "Players",
			titleIcon = UI.Characters,
			size = UDim2.fromScale(0.7, 0.7),
			pos = UDim2.fromScale(0.5, 0.5),
			ratio = 1.6,
			condition = UIReducer.CurrentUI == FramesConstants.Characters,
			...
			Container = Roact.createElement("Frame", {
				...
				ScrollingFrame = Roact.createElement("ScrollingFrame", {
					...
					Roact.createFragment(Characters),
				}),
			}),
		}),
	})
end
Characters = RoactHooks.new(Roact)(Characters)
return Characters
