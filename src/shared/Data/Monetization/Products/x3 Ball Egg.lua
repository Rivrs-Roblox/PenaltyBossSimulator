...
return table.freeze({
	[3583663845] = {
		...
		["BeforeCheck"] = function(self, userId)
			...
			if GetTableLength(data.Inventory.Pets) + 3 > data.Inventory.Storage.Stored then
		...
		["Purchased"] = function(self, userId)
			local _, Pets = EggsService:Hatch(Players:GetPlayerByUserId(userId), 3, "BallEgg", {}, true)
