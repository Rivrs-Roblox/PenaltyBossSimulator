...
return table.freeze({
	[3583663937] = {
		...
		["BeforeCheck"] = function(self, userId)
			...
			if GetTableLength(data.Inventory.Pets) + 8 > data.Inventory.Storage.Stored then
		...
		["Purchased"] = function(self, userId)
			local _, Pets = EggsService:Hatch(Players:GetPlayerByUserId(userId), 8, "BallEgg", {}, true)
			self.Client.EggPurchased:Fire(Players:GetPlayerByUserId(userId), Pets, "BallEgg")
		end,
	},
})
